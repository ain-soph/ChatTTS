...
def train_autoencoder(
    ...
    for _epoch in range(epochs):
        ...
        for batch in iterator:
            ...
            audio_quantized_latents = audio_quantized_latents * mel_attention_mask[
                :, ::2
            ].unsqueeze(1)
            gen_mel_specs = dvae_decode(
...
def train_gpt(
    ...
    speaker_embeds = {
        speaker: chat._sample_random_speaker().requires_grad_(
            train_module
            in [TrainModule.GPT_ALL, TrainModule.GPT_SPEAKER, TrainModule.SPEAKER],
        )
    ...
    match train_module:
        ...
        case TrainModule.SPEAKER:
            train_params = list(speaker_embeds.values())
            optimizer = torch.optim.Adam(
                train_params, lr=1e-2, weight_decay=0, betas=[0.9, 0.95], eps=1e-5
            )
    ...
    for _epoch in range(epochs):
        ...
        for batch in iterator:
            ...
            dvae_audio_input_ids[~mel_attention_mask[:, ::2].bool()] = (
                AUDIO_PAD_TOKEN_ID
            )
